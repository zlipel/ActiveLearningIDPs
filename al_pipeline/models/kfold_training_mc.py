...
def KFold_GPR_multitask(features_file, labels_file, label_columns = ['exp_density', 'diff'], k=5, epochs=10, \
    ...
    if exploration == 'kriging_believer' or exploration == 'constant_liar_min' or exploration == 'constant_liar_mean' or exploration == 'constant_liar_max':
        ...
        features_train_df.to_csv(os.path.join(gen_folder, f"{featname}_NORM_{ehvi_var}_{exploration}_{transform}_MC.csv"), index=False)
        labels_train_df.to_csv(os.path.join(gen_folder, f"{labelname}_NORM_{ehvi_var}_{exploration}_{transform}_MC.csv"), index=False)
